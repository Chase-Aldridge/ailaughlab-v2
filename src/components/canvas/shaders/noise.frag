...
void main() {
  ...
  float distortionOffset = uDistortion * 0.02;
  uv.x += snoise(vec3(uv * 3.0, uTime * 0.5)) * distortionOffset;
  uv.y += snoise(vec3(uv * 3.0 + 100.0, uTime * 0.5)) * distortionOffset;

  // Multi-octave noise
  float n1 = snoise(vec3(uv * 1.5, uTime * 0.08)) * 0.5 + 0.5;
  float n2 = snoise(vec3(uv * 3.0, uTime * 0.12 + 10.0)) * 0.5 + 0.5;
  float n3 = snoise(vec3(uv * 6.0, uTime * 0.06 + 20.0)) * 0.5 + 0.5;
